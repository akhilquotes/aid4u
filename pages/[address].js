...
export default function Detail({ Data, DonationsData }) {
  ...
  useEffect(() => {
    const Request = async () => {
      ...
      const MyAllDonations = await contract.listDonations();

      setMydonations(
        MyAllDonations.map((e) => {
          return {
            donar: e.donar,
            amount: ethers.utils.formatEther(e.amount),
      ...
      <RightContainer>
        ...
        <FundsData>
          <Funds>
            ...
            <FundText>{Data.receivedAmount} OKT</FundText>
          </Funds>
        </FundsData>
        <Donated>
          <LiveDonation>
            ...
            {DonationsData.map((e) => {
              return (
                <Donation key={e.timestamp}>
                  ...
                  <DonationData>{e.amount} OKT</DonationData>
                  <DonationData>
...
export async function getStaticProps(context) {
  ...
  const AllDonations = await contract.listDonations();
  const Data = {
